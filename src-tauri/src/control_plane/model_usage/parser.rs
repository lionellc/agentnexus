...
pub(super) fn parse_session_file(
    ...
    loop {
        ...
        facts.push(ModelUsageFactDraft {
            ...
            total_duration_ms: extracted.total_duration_ms,
            first_token_ms: extracted.first_token_ms,
            source: file.source.clone(),
            source_path: file.path.clone(),
...
pub(super) fn extract_model_usage_event(value: &Value) -> Option<ExtractedModelUsageEvent> {
    ...
    let total_duration_ms = find_first_i64(
        value,
        &[
            "total_duration_ms",
            "totalDurationMs",
            ...
            "metrics.latency_ms",
            "metrics.latencyMs",
            "payload.metrics.total_duration_ms",
            "payload.metrics.totalDurationMs",
    ...
    let first_token_ms = find_first_i64(
        value,
        &[
            "first_token_ms",
            "firstTokenMs",
            ...
            "payload.first_text_delta_ms",
            "payload.firstTextDeltaMs",
            "payload.time_to_first_token_ms",
            "payload.timeToFirstTokenMs",
            "payload.ttft_ms",
            "payload.ttftMs",
            "payload.first_response_ms",
            "payload.firstResponseMs",
            "metrics.first_token_ms",
            "metrics.firstTokenMs",
            "metrics.ttft_ms",
            "metrics.ttftMs",
            "payload.metrics.first_token_ms",
            "payload.metrics.firstTokenMs",
            "payload.metrics.ttft_ms",
            "payload.metrics.ttftMs",
        ],
    );
