...
pub(super) fn run_anthropic(
    ...
    let url = http::endpoint(&input.base_url, "/v1/messages");
    let headers = match http::headers(&[
        ("x-api-key", input.api_key.clone()),
...
#[cfg(test)]
pub(super) fn parse_anthropic_json_for_test(raw: &str) -> ProtocolResponse {
    parse_anthropic_json(
        raw,
        Some(StatusCode::OK),
        FIRST_RESPONSE,
